...
detector = cv2.CascadeClassifier(str(BASE_DIR) + '\\fr\\haarcascade_frontalface_default.xml')
...
class FaceRecognition:    
    ...
    def faceDetect(self, Entry1,):
        ...
        while(True):
            ...
            for (x,y,w,h) in faces:
                ...
                cv2.imwrite(str(BASE_DIR) +'\\fr\\dataset\\User.' + str(face_id) + '.' + str(count) + ".jpg", gray[y:y+h,x:x+w])
    ...
    def trainFace(self):
        ...
        recognizer.save(str(BASE_DIR) + '\\fr\\trainer\\trainer.yml') # recognizer.save() worked on Mac, but not on Pi
    ...
    def recognizeFace(self):
        recognizer.read(str(BASE_DIR) + '\\fr\\trainer\\trainer.yml')
        cascadePath = str(BASE_DIR) + '\\fr\\haarcascade_frontalface_default.xml'
